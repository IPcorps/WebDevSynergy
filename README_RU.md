...
WebDevSynergy - это простое, и в тоже время высоко-конфигурируемое окружение, которое позволит вести веб разработку легче и быстрее как для новичка, так и для продвинутого разработчика. Все что требуется для работы данного окружения - установленная [**NodeJS**](https://nodejs.org). Основные особенности:
